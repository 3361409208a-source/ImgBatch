  ...
  return (
    <div className="flex items-center gap-2 px-4 py-2 bg-[color:var(--color-surface-2)] border-b border-border">
      ...
      <select
        ...
        className="field-sm cursor-pointer min-w-[7rem]"
      >
        {SIZES.map((sz) => (
      ...
      <div className="flex items-center gap-1.5">
        <span className="label-muted">{t('filter_dim')}</span>
        <input
          type="text"
          placeholder="W"
      ...
      <button type="button" onClick={() => void s.applyFilter()} className="btn-primary h-7 text-xs">
        {t('filter')}
      </button>
      <button
        type="button"
